...
func main() {
	...
	app.Initialize("src/github.com/davidrenne/goCoreCreateAppGeneration", "webConfig.json")
	settings.Initialize()
	...
	serverSettings.Initialize(os.Getenv("GOPATH")+"/src/github.com/davidrenne/goCoreCreateAppGeneration", "webConfig.json")
...
`, inputFields[i].Name, capCamel, inputFields[i].Name, inputFields[i].Name)
	...
	if hasNameField {
		...
		`)
		utils.ReplaceTokenInFile(br, "-QUERIES-", `"github.com/davidrenne/goCoreCreateAppGeneration/queries"`)
	} else {
		utils.ReplaceTokenInFile(br, "-COPY_NAME-", "")
